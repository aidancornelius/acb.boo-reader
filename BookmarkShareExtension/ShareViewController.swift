...
class ShareViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        NSLog("ShareViewController loaded")
        loadSharedData()
    }
                ...
                itemProvider.loadItem(forTypeIdentifier: kUTTypeURL as String, options: nil) { [weak self] (url, error) in
                    DispatchQueue.main.async {
                        if let shareURL = url as? URL {
                            NSLog("Shared URL: \(shareURL.absoluteString)")
                            self?.urlTextField.text = shareURL.absoluteString
                        } else if let error = error {
                            NSLog("Error loading shared URL: \(error.localizedDescription)")
                        }
                    }
                }
            } else {
    ...
    @IBAction func saveButtonTapped(_ sender: Any) {
        ...
        NSLog("Attempting to save bookmark: URL = \(url), Comment = \(comment)")
    ...
    private func saveBookmark(url: String, comment: String) {
        guard let sharedDefaults = sharedDefaults else {
            NSLog("Error: Could not access shared UserDefaults")
            return
        }
        ...
        if var bookmarks = sharedDefaults.array(forKey: "pendingBookmarks") as? [[String: Any]] {
            ...
            NSLog("Added bookmark to existing pendingBookmarks. Total count: \(bookmarks.count)")
        } else {
            sharedDefaults.set([bookmark], forKey: "pendingBookmarks")
            NSLog("Created new pendingBookmarks with 1 bookmark")
        }
        
        NSLog("Bookmark saved to shared UserDefaults")
    }
}
